...
const Player = function(ctx, x, y, gameArea) {
    ...
    sprite.setSequence(sequences.idleDown)
          ...
          .useSheet("/photo/player_sprite.png");
    ...
    const move_request = function(dir) {
        if (dir >= 1 && dir <= 4 && dir != direction) {
            switch (dir) {
                case 1: Socket.move(1); break;
                case 2: Socket.move(2); break;
                case 3: Socket.move(3); break;
                case 4: Socket.move(4); break;
            }
        }
    };
    ...
    const stop_request = function(dir) {
        
        console.log("send stop request to server",dir);
        switch (dir) {
            case 1: Socket.stop(1); break;
            case 2: Socket.stop(2); break;
            case 3: Socket.stop(3); break;
            case 4: Socket.stop(4); break;
        }
        
    };
    const stop = function(dir) {
        console.log("stop function called",dir);
        if (dir >= 1 && dir <= 4 ) {
            console.log("stop function called and get in",dir);
            switch (dir) {
    ...
    // turn player's speed back to normal
    const speedUp = function() {
        speed = 150;
    };

    // hit trap -> slows down player
    ...
    const shoot_request = function() {
        console.log("shoot_request function called and emit shoot");
        Socket.shoot(facing);
    };
